...
int FD_alloc1ddata(FD* fd)
{
  int ntot;
  ntot=fd->np*(fd->ni > 1 ? fd->ni : 1);
  fd->data=(double2*)malloc(sizeof(double2)*(ntot+1));
  if (!fd->data) {
...
void FD_zero(FD* fd)
{
  ...
  memset(fd->data,0,sizeof(double2)*(fd->np*(fd->ni > 1 ? fd->ni : 1)+1));
}
...
*/
       ...
       if (fd->format == FD_FORMAT_TEXT) {
         ...
         if (fd->prec == FD_PREC_SINGLE) {
           ...
           int nb;
           if ((nb=fread(&(fd->data[1]),sizeof(double2),ntot,fp)) != ntot) {
    ...
    if (q) {
       *q=0; q++;
	   ...
	   while (*dd == '\n') dd++;
           ...
           int nb;
	   if (strlen(dd) < ntot*sizeof(double2)) {
             fprintf(stderr,"FD_read: read %d but needed %d complex numbers\n",nb,ntot);
             exit(1);           
           } else {
	     memcpy(&fd->data[1], dd, ntot*sizeof(double2));
	   }
         }
       }
       strcpy(buf,"");
...
FD* FD_dup(FD* fd)
{
  ...
  to->sw1=fd->sw1;
  FD_alloc1ddata(to);
  memcpy(to->data,fd->data,sizeof(double2)*(to->np*(to->ni > 1 ? to->ni : 1)+1));
  return to;
}

FD* FD_data2fd(char* file,double2* data,int np,int ni,double sw,double sw1)
{
  FD* fd;
  ...
  fd->sw1=sw1;
  FD_alloc1ddata(fd);
  memcpy(fd->data,data,sizeof(double2)*(fd->np*(fd->ni > 1 ? fd->ni : 1)+1));
  return fd;
}
