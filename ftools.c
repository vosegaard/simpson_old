...
/* Intel MKL defines this function, so exclude from MKL build */
#ifndef INTEL_MKL
void dcopy(double2* vec,double2* vec2,int nvec)
{
  int i;
...
void dscalephase(double2* vec,double2* vec2,int nvec,double scale,double offset,
                 double rp,double lp)
{
  double w,phase;
...
/* vec and vec2 can be the same data. */
void dphase(double2* vec,double2* vec2,int nvec,double rp,double lp)
{
  double w,phase;
  double cosph,sinph,re,im;
  int i;
...
/* vec and vec2 can be the same data. */
void dscale(double2* vec,double2* vec2,int nvec,double scale,double offset)
{
  int i;
...
void daddlb(double2* vec,int nvec,double sw,double lineb,double
...
void daddlb2d(double2* vec,int np,int ni,double sw,double lineb,double glfrac,
...
int fdupzero(int fidN)
{
  ...
  f=FD_dup(fd[fidN]);
  if (!f) return -1;
  memset(f->data,0,sizeof(double2)*(f->np*(f->ni > 1 ? f->ni : 1)+1));
  return fnew(f);
}
...
int tclFZerofill(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  if (argc == 4) {
    ...
    if (npz > np || niz > f->ni) {
      d=(double2*)malloc(sizeof(double2)*(niz*npz+1));
      if (!d)
        return TclError(interp,"fzerofill: unable to allocate %d complex numbers\n",niz*npz);
      memset(d,0,sizeof(double2)*(niz*npz+1));
      for (i=0;i<f->ni;i++) {
        memcpy(&d[i*npz+1],&f->data[i*np+1],sizeof(double2)*np);
      }
      free(f->data);
      f->ni=niz;
      f->np=npz;
    ...
    if (npz > f->np) {
      d=(double2*)malloc(sizeof(double2)*(npz+1));
      if (!d)
        return TclError(interp,"fzerofill: unable to allocate %d complex numbers\n",npz);

      memset(d,0,sizeof(double2)*(npz+1));
      memcpy(&d[1],&f->data[1],sizeof(double2)*f->np);
      free(f->data);
      f->np=npz;
      f->data=d;      
    }
...
*/
    ...
    daddlb((double2*)f->data,f->np,f->sw,lineb,glfrac,sym,top);
  }
  return TCL_OK;
}
...
int tclFCopy(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  dcopy((double2*)(fd[fidN]->data),(double2*)(fd[fidN2]->data),fd[fidN]->np*(fd[fidN]->ni > 1 ? fd[fidN]->np : 1));
  return TCL_OK;
} 
...
void addpoint(double2* vec, int i, int np, complx w)
{
  int ii = i;
  while (ii < 1) ii+= np;
  while (ii > np) ii-= np;
  ...
  if (is_lp == 0 && is_rp == 0) {
    if (fd[fidN]->ni > 1) {
      for (j=0; j<fd[fidN]->ni; j++) {
        vec = (double2*) &fd[fidN]->data[j*fd[fidN]->np];
        dscale(vec,vec,nvec,scale,offset);
      }
    } else {
      vec = (double2*) fd[fidN]->data;
      dscale(vec,vec,nvec,scale,offset);
    }
  } else {
    if (fd[fidN]->ni > 1) {
...
int tclFExtract(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  xto   = FD_FREQ(f,ito);
  newnp= ito - ifrom;
  newdat=(double2*)malloc(sizeof(double2)*(newnp+1));
  if (!newdat)
...
void daddrms(double2* vec, double2* vec2,int from,int to,int part,double* sumrms,double* sumint)
{
  int i;
...
void daddscale(double2* vec, double2* vec2,int from,int to,int part,double* sumxx,double* sumxy)
{
  int i;
...
int tclFZero(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  for (i=0;i<npar;i++) {
    ...
    if (i2 < 1) i2=1; else if (i2 > nvec) i2=nvec;
    if (i1 < i2) {
      memset(&vec[i1],0,sizeof(double2)*(i2-i1+1));
    }
    free(par2);
  }
  free(par);
...
double2 pol3int(double x,double x1,double dx,
            double2 y1, double2 y2, double2 y3, double2 y4)
{
...
int setnewnp(Tcl_Interp *interp, FD *f, int newnp) {
  int i,j,np,np2;
  ...
  if (f->ni > 1) {
    newdata=(double2*)malloc(sizeof(double2)*(newnp+1)*f->ni);
  } else {
...
*/
  ...
  return setnewnp(interp, fd[fidN], newnp);
}

int tclFSaveData(ClientData data, Tcl_Interp *interp, int argc, char *argv[]) {
  int i, j, fidN, tmp;
  FD *f;
  FILE *fpr, *fpi;
  if (argc < 2)
    return TclError(interp,"Usage: fsavedata <data set>");
  
  if (Tcl_GetInt(interp,argv[1],&fidN) == TCL_ERROR)
    return TclError(interp,"fsavedata: argument 1 must be integer <data set>");
  
  if (fidN < 1 || fidN > nfd || fd[fidN] == NULL)
    return TclError(interp,"fsavedata: data set %d was not previously loaded\n",fidN);
  ...
  if (f->ni > 1) {
    ...
    fpi = fopen("2ii", "w");
    if (!fpr || !fpi) 
      return TclError(interp, "fsavedata: unable to open files '2rr' and '2ii'\n");
    for (j=0; j<f->ni; j++) {
      for (i=f->np-1; i>=0; i--) {
        int ii = i==0 ? (j+1)*f->np:j*f->np+i;
        tmp = (int)f->data[ii].re;
        fwrite(&tmp, sizeof(int), 1, fpr);
        tmp = (int)f->data[ii].im;
        fwrite(&tmp, sizeof(int), 1, fpi);
      }
    }
    fclose(fpr);
    fclose(fpi);
    ...
    fpi = fopen("1i", "w");
    if (!fpr || !fpi) 
      return TclError(interp, "fsavedata: unable to open files '1r' and '1i'\n");
    for (i=f->np-1; i>=0; i--) {
      int ii = i==0 ? f->np:i;
      tmp = (int)f->data[ii].re;
      fwrite(&tmp, sizeof(int), 1, fpr);
...
int tclFCreate(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  char buf1[16], buf2[128];
  char *real_data = NULL, *imag_data = NULL;
  
  if ((argc % 2) == 0)
    return TclError(interp,"Usage: <data set> fcreate -np v -sw v ?-type (spe|fid)? ?-ref v? ?-ni v? ?-sw1 v? ?-ref1 v? -real_data <data> -imag_data <data>" );
  ...
  for (ac=1;ac<argc;ac += 2) {
    if (!strcmp(argv[ac],"-ref")) {
      ...
      if (Tcl_GetInt(interp,argv[ac+1],&(f->ni)) == TCL_ERROR)
         return TclError(interp,"fcreate: argument %d must be integer",ac+1);
    } else if (!strcmp(argv[ac],"-real_data")) {
      real_data = argv[ac+1];
    } else if (!strcmp(argv[ac],"-imag_data")) {
      imag_data = argv[ac+1];
  ...
  if (real_data != NULL) {
//    FD_alloc1ddata(f);
    FILE *fpr = fopen(real_data, "r");
    FILE *fpi = NULL;
    int start, end, size;
    if (fpr == NULL) 
      return TclError(interp, "fcreate: unable to open file '%s'\n", real_data);
    start = ftell(fpr);
    ...
    if (size != f->np)
      return TclError(interp, "fcreate: number of points (%d) does not match number of points in file '%s' (%d)\n",
                      f->np, real_data, size);
    
    if (imag_data != NULL) {
      fpi = fopen(imag_data, "r");
      if (fpi != NULL) {
        start = ftell(fpi);
    ...
    for (i=f->np; i>=1; i--) {
      ii = i == f->np ? 1:i+1;
      fread(&d, sizeof(int), 1, fpr);
      f->data[ii].re = (double)d;
      if (fpi != NULL) {
        fread(&d, sizeof(int), 1, fpi);
        f->data[ii].im = (double)d;
      }
    }
  }
  sprintf(buf1, "%d", fidN);
...
void fft2d(double2* data,int np,int ni,double sw,double sw1,double
     rp,double lp,double rp1,double lp1,double dlp,int phsens)
{
  void fft(double2* data,int np,int is);
  void dphase(double2* vec,double2* vec2,int nvec,double rp,double lp);  
  int i,j,ni2;
  double2 *d;
  ...
  d=(double2*)malloc(sizeof(double2)*(ni+1));
  if (!d) {
...
int tclFFt1d(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  if (argc == 3) {
    if (strcmp(argv[2],"-inv"))
      return TclError(interp,"fft: unknown option '%s' (must be -inv or noting)\n",argv[2]);
    for (i=0; i<f->ni; i++) {
      fft((double2*)&f->data[i*f->np],f->np,-1);
    }
  } else {
    for (i=0; i<f->ni; i++) {
      fft((double2*)&f->data[i*f->np],f->np,1);
...
int tclFTranspose(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  d = (double2*)malloc(sizeof(double2)*(f->np*f->ni+1));
  if (!d) {
...
int tclFAdd(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  v1=(double2*)(fd[fidN]->data);
  v2=(double2*)(fd[fidN2]->data);
  for (i=1;i<=np;i++) {
     v1[i].re += v2[i].re;
     v1[i].im += v2[i].im;
  }
...
void fbc(double2* src,double2* dst,int np,int order,RANGE* range,int nrange,int skip)
{
...
int tclFSsbint(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  memset(fn->data,0,sizeof(double2)*(fn->np+1));
  ...
  for (i=1;i<=f->np;i++) {
     ...
     ((double2*)fn->data)[j].re +=((double2*)f->data)[i].re;
     ((double2*)fn->data)[j].im +=((double2*)f->data)[i].im;
  }
...
void dsave2d_reim(double2* data,int ni,int np,char* name,double scale,double autoscale) 
{
  FILE* fp;
  int i,j,k;
...
void dsave2d_ps(double2* data,int ni,int np,char* name,double scale,int autoscale) 
{
  FILE* fp;
...
void dsave2d_ppm(double2* data,int ni,int np,char* name,double scale,int autoscale) 
{
  FILE* fp;
  int i,j,k,l;
...
int fint(FD *f, double x, double y, double2* v)
{
  int xi, yi;
  double wx, wy;
  double2 a, b, c, d;
...
{
  Tcl_CreateCommand(interp,"fabs",tclFAbs,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fload",tclFLoad,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"floaddata",tclFLoaddata,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"funload",tclFUnload,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fsave",tclFSave,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fphase",tclFPhase,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fscale",tclFPhase,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fdup",tclFDup,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fdupzero",tclFDupZero,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"frms",tclFRms,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"frealrms",tclFRealrms,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fautoscale",tclFAutoscale,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fextract",tclFExtract,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fzero",tclFZero,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fzerofill",tclFZerofill,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"frev",tclFRev,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fget",tclFGet,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fset",tclFSet,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fcreate",tclFCreate,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fft",tclFFt,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"faddpeaks",tclFAddpeaks,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fint",tclFInt,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fssbint",tclFSsbint,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fadd",tclFAdd,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fsub",tclFSub,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fcopy",tclFCopy,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"faddlb",tclFAddlb,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"findex",tclFIndex,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fx",tclFX,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"ffindpeaks",tclFFindPeaks,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fmaxheight",tclFMaxheight,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fsetindex",tclFSetIndex,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fplot2d",tclFPlot2d,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"faddtriangle",tclFAddtriangle,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"freconstruct",tclFReconstruct,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fcompare",tclFCompare,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);

  Tcl_CreateCommand(interp,"fnewnp",tclFNewnp,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fsavedata",tclFSaveData,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);

  Tcl_CreateCommand(interp,"fread",tclFRead,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);

  Tcl_CreateCommand(interp,"fft1d",tclFFt1d,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"ftranspose",tclFTranspose,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);


/* Must find replacement for numerical recipes code to work */
  Tcl_CreateCommand(interp,"fbc",tclFBc,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
  Tcl_CreateCommand(interp,"fsmooth",tclFSmooth,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
}
