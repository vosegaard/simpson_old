...
/****
 *  ZT: sets proper offsets to Pulse structure contained in Sim structure
 ****/
void set_inhom_offsets(Sim* s,double zoffnominal)
{
  int i, Nchan;
  double chanoffset;
  double *inh_offset;
  
  inh_offset = s->P->inhom_offset;
  Nchan = s->ss->nchan;
...
Tcl_CreateCommand(interp,"zgrad_shape_create",tclZgradCreate,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
Tcl_CreateCommand(interp,"free_zgrad",tclZgradFree,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
Tcl_CreateCommand(interp,"load_zgrad",tclZgradLoad,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
Tcl_CreateCommand(interp,"zgrad2list",tclZgradList,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);
}
